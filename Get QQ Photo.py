        ...
        while not self.work_queue.empty():  #队列非空时，一直循环
            ...
            try:
                ...
                if 'Content-Type' in r.info():
                    fileName = os.path.join(self.fold, replace(
...
def checklist(hostuin):
    ...
    for i in range(4):
        res = Http("http://" + alist[
            i] + ".photo.qq.com/fcgi-bin/fcg_list_album_v2?inCharset=gbk&outCharset=gbk&hostUin=%s&notice=0&callbackFun=&format=jsonp&plat=qzone&source=qzone&appid=4&uin=%s&t=%s&g_tk=%s" % (
                   hostuin, uin, random.Random().random(), gtk), "GBK")
        if res.find("没有权限") != -1:
            return "没有权限"
            break
        else:
...
def getphotolist(hostuin, alist, plist, idcnum):
    res = Http(
        "http://" + alist + ".photo.qq.com/fcgi-bin/fcg_list_album_v2?inCharset=gbk&outCharset=gbk&hostUin=%s&notice=0&callbackFun=&format=jsonp&plat=qzone&source=qzone&appid=4&uin=%s&t=%s&g_tk=%s" % (
        hostuin, uin, random.Random().random(), gtk), "GBK")
                ...
                if int(num) < len(album) + 1:
                    ...
                    if album[num]["total"] != 0:
                        ...
                        for x in range(ceil(album[num]["total"] / 100)):
                            pageStart = x * 100
                            res = Http(
                                "http://" + plist + ".photo.qq.com/fcgi-bin/cgi_list_photo?g_tk=%s&callback=shine0_Callback&mode=0&idcNum=%s&hostUin=%s&topicId=%s&noTopic=0&uin=%s&pageStart=%s&pageNum=100&singleurl=1&notice=0&appid=4&inCharset=gbk&outCharset=gbk&source=qzone&plat=qzone&outstyle=json&format=jsonp&json_esc=1&callbackFun=shine0&_=%s" % (
                                gtk, idcnum, hostuin, album[num]["id"], uin, pageStart, int(time.time() * 1000)), "GBK")
                            ...
                            if res.find("对不起，回答错误") != -1:
                                ...
                                if match:
                                    ...
                                    res = Http(
                                        "http://" + p + "photo.qq.com/cgi-bin/common/cgi_view_album_v2?inCharset=gbk&outCharset=gbk&hostUin=%s&notice=0&callbackFun=&format=jsonp&plat=qzone&source=qzone&appid=4&uin=%s&albumId=%s&singleUrl=1&t=%s&verifycode=&question=%s&answer=%s&g_tk=%s" % (
                                        hostuin, uin, album[num]["id"], int(time.time() * 1000), parse.quote(question),
                                        answer, gtk), "GBK")
                                    if res.find("对不起，回答错误") != -1:
                                        print("提示：————————————【对不起，回答错误!】")
                                        break
...
class qqlogin:
    ...
    def GetVerifyCode(self):
        #判断是否需要验证码
        check = Http("http://check.ptlogin2.qq.com/check?regmaster=&uin=%s&appid=%s&r=%s" % (
        self.uin, appid, random.Random().random()))
    ...
    def Login(self, uid, password, verifycode):
        ...
        url = "http://ptlogin2.qq.com/login?ptlang=2052&u=" + uid + "&p=" + p + "&verifycode=" + verifycode[
            1] + "&css=http://imgcache.qq.com/ptcss/b2/qzone/15000101/style.css&mibao_css=m_qzone&aid=" + appid + "&u1=http%3A%2F%2Fimgcache.qq.com%2Fqzone%2Fv5%2Floginsucc.html%3Fpara%3Dizone&ptredirect=1&h=1&from_ui=1&dumy=&fp=loginerroralert&action=2-14-13338&g=1&t=1&dummy="
